...
function($scope, $rootScope, $uibModalInstance, apiService, $dialogs, data){
	...
	$scope.gradeWeight = {};
	$scope.reportCardTypes = ["Standard","Standard-v.2","Standard-v.3","Kindergarten","Playgroup"];
	$scope.examTypes = [];
...
function($scope,$rootScope,$uibModalInstance,apiService,data){
		...
		var apiError = function(response,status)
		{
			var result = angular.fromJson( response );
			$scope.error = true;
			var msg = ( result.data.indexOf('"U_exam_type_per_category"') > -1 ? 'This exam type has already been entered for this class.' : result.data);
			$scope.errMsg = msg;
		}
			...
			'<div class="modal-body cleafix">' +
				'<ng-form name="catDialog" class="form-horizontal modalForm" novalidate role="form">' +
					...
					'<div class="form-group ng-class="{ \'has-error\' : catDialog.class_cat.$invalid && (catDialog.class_cat.$touched || catDialog.$submitted) }">' +
						'<label for="class_cat" class="col-sm-3 control-label">Class</label>' +
						'<div class="col-sm-9">' +
