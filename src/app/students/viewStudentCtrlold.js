...
function($scope, $rootScope, $uibModalInstance, apiService, $dialogs, FileUploader, $timeout, $filter, data){
	...
	$scope.getStudentDetails = function(student_id)
	{
        ...
        if( $scope.transportRoutes ) {
          ...
          if( $scope.student.transport_route !== undefined ) $scope.showTransport = true;
			...
			var studentDOB = new Date($scope.student.dob);
			var studentDOByear = studentDOB.getFullYear();
			$scope.student.age = age = yyyy - studentDOByear;
			var age = yyyy - studentDOByear;
	...
	*/
	...
	$scope.addInvoice = function()
	{
		...
		var dlg = $dialogs.create('http://localhost/eduweb/app/fees/invoiceForm.html','invoiceFormCtrl',{selectedStudent:$scope.student},{size: 'lg',backdrop:'static'});
	...
	$scope.viewInvoice = function(item)
	{
		...
		var domain = window.location.host;
		var dlg = $dialogs.create('http://localhost/eduweb/app/fees/invoiceDetails.html','invoiceDetailsCtrl',item,{size: 'md',backdrop:'static'});
		dlg.result.then(function(result){
			// update invoices
			getStudentBalance();
			getInvoices();
	...
	$scope.getInvoice = function(invoice)
	{
		...
		var domain = window.location.host;
		var dlg = $dialogs.create('http://localhost/eduweb/app/fees/invoice.html','invoiceCtrl',data,{size: 'md',backdrop:'static'});
	...
	$scope.printStatement = function()
	{
		...
		var domain = window.location.host;
		var newWindowRef = window.open('http://localhost/eduweb/#/fees/statement/print');
	...
	$scope.addPayment = function()
	{
		// open dialog
		var domain = window.location.host;
		var dlg = $dialogs.create('http://localhost/eduweb/app/fees/paymentForm.html','paymentFormCtrl',{selectedStudent:$scope.student},{size: 'lg',backdrop:'static'});
	...
	$scope.getReceipt = function(payment)
	{
		...
		var domain = window.location.host;
		var dlg = $dialogs.create('http://localhost/eduweb/app/fees/receipt.html','receiptCtrl',data,{size: 'md',backdrop:'static'});
	}
	...
	$scope.viewPayment = function(payment)
	{
		...
		var domain = window.location.host;
		var dlg = $dialogs.create('http://localhost/eduweb/app/fees/paymentDetails.html','paymentDetailsCtrl',payment,{size: 'lg',backdrop:'static'});
		dlg.result.then(function(payment){
			// update invoices
			getStudentBalance();
			getPayments();
	...
	$scope.addFeeItem = function()
	{
		// open dialog
		var domain = window.location.host;
		var dlg = $dialogs.create('http://localhost/eduweb/app/fees/feeItemForm.html','feeItemFormCtrl',undefined,{size: 'md',backdrop:'static'});
	...
	$scope.addReportCard = function()
	{
		...
		var domain = window.location.host;
		var dlg = $dialogs.create('http://localhost/eduweb/app/exams/reportCard.html','reportCardCtrl',data,{size: 'lg',backdrop:'static'});
		dlg.result.then(function(examMarks){
			$scope.getStudentReportCards();
		},function(){
			$scope.getStudentReportCards();
