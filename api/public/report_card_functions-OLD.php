...
$app->get('/getExamMarksforReportCard/:student_id/:class/:term(/:teacherId)', function ($studentId,$classId,$termId,$teacherId=null) {
	...
	try
	{
		...
		//this needs editing as positions are not appearing for some students
		...
		$sth5 = $db->prepare("SELECT DISTINCT SUM(mark) AS average_grade,exam_type,et.creation_date FROM app.exam_marks em
INNER JOIN app.class_subject_exams cse
	INNER JOIN app.exam_types et ON cse.exam_type_id = et.exam_type_id
	INNER JOIN app.class_subjects
	...
	INNER JOIN app.classes ON class_subjects.class_id = classes.class_id
				ON cse.class_subject_id = class_subjects.class_subject_id AND class_subjects.active is true
				ON em.class_sub_exam_id = cse.class_sub_exam_id
WHERE em.student_id = :studentId AND grade_weight = 100 AND subjects.use_for_grading is true
GROUP BY exam_type,et.creation_date
ORDER BY et.creation_date ASC");
		$sth5->execute(  array(':studentId' => $studentId) );
		$graphPoints = $sth5->fetchAll(PDO::FETCH_OBJ);

		//stream position of student
		// $sth6 = $db->prepare("");
		// $sth6->execute(  array(':studentId' => $studentId) );
		// $streamPosition = $sth6->fetchAll(PDO::FETCH_OBJ);
		...
		// $results->streamPosition = $streamPosition;
